...
export function ImageModal({ images, initialIndex, isOpen, onClose }: ImageModalProps) {
  ...
  return (
    <Modal open={isOpen} onOpenChange={onClose}>
        <ModalContent className="max-w-6xl max-h-[90vh] w-full h-full p-0 bg-background border border rounded-lg overflow-hidden sm:max-w-[95vw] sm:max-h-[95vh]">
          ...
          {images.length > 1 && (
            <Button
              variant="ghost"
              size="icon"
              className="absolute left-2 md:left-6 top-1/2 -translate-y-1/2 z-50 text-white hover:text-primary-foreground hover:bg-primary bg-black/20 rounded-full border border h-10 w-10 md:h-12 md:w-12 transition-all duration-200"
          ...
          {images.length > 1 && (
            <Button
              variant="ghost"
              size="icon"
              className="absolute right-2 md:right-6 top-1/2 -translate-y-1/2 z-50 text-white hover:text-primary-foreground hover:bg-primary bg-black/20 rounded-full border border h-10 w-10 md:h-12 md:w-12 transition-all duration-200"
              onClick={nextImage}
            >
          ...
          {images.length > 1 && (
            <div className="absolute bottom-16 md:bottom-20 left-1/2 -translate-x-1/2 z-40 hidden md:block">
              <div className="flex justify-center space-x-3 max-w-full overflow-x-auto bg-card p-3 rounded-full border border">
